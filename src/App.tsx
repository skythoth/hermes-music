...
function HermesApp() {
  ...
  return (
    <div className="app-root">
      <div className="view-switch">
        ...
        {user && (
          <span style={{ marginLeft: 'auto', color: 'var(--muted)', fontSize: 12.5, display: 'flex', alignItems: 'center', gap: 8 }}>
            {user.display_name}
            <button className="icon-btn" title="로그아웃" onClick={() => { clearAuth(); window.location.reload(); }}>
              ×
            </button>
          </span>
        )}
      ...
      {view === "desktop"
        ? <DesktopApp H={H} userName={user?.display_name} />
        : <div className="mob-stage"><MobileApp H={H} /></div>}
    </div>
  );
}
